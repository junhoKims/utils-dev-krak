...
describe('curry', () => {
  test('first', () => {
    function logger(mode: 'DEBUG' | 'ERROR' | 'WARN', initMsg: string, errMsg: string, lineNo: number) {
      if (mode === 'DEBUG') console.debug(initMsg, errMsg + 'at lint: ' + lineNo);
      else if (mode === 'ERROR') console.error(initMsg, errMsg + 'at lint: ' + lineNo);
      else if (mode === 'WARN') console.warn(initMsg, errMsg + 'at lint: ' + lineNo);
      else throw 'Wrong';
    }

    let debugLogger = curry(logger)('DEBUG')('Init Debug Error');
    let errLogger = curry(logger)('ERROR')('Init Error Error');
